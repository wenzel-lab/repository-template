# [Project Name] [![Open Source Love](https://badges.frapsoft.com/os/v1/open-source.svg?v=103)](https://github.com/ellerbrock/open-source-badges/)

**Brief one-sentence description.**

This project is a free and open-source design for [brief description]. For more resources on open-source hardware for bioimaging, see [LIBRE Hub](https://github.com/LIBREhub) and the [Wenzel Lab website](https://wenzel-lab.github.io/en/).

[Optional: Add a hero image]

---

## Overview

[2-3 sentences describing what this is and why it exists]

**Key Features:**
- Feature 1
- Feature 2
- Feature 3

---

## Bill of Materials (BOM)

| Component | Specification | Quantity | Supplier | Cost |
|-----------|--------------|----------|----------|------|
| Component 1 | Spec | X | [Link] | $XX |
| Component 2 | Spec | X | [Link] | $XX |

**Total Cost**: ~$XXX

*Detailed BOM available in [BOM.csv](Build/BOM/BOM.csv)*

---

## Getting Started

### Prerequisites
- [Tool/Software 1]
- [Tool/Software 2]

### Assembly

1. **Step 1**: [Description]
2. **Step 2**: [Description]
3. **Step 3**: [Description]

*For detailed assembly instructions, see [Assembly Guide](Docs/Assembly/assembly-guide.md)*

---

## Software/Firmware

[If applicable]

**Installation:**
```bash
[commands]
```

**Usage:**
```bash
[commands]
```

*See [Software README](Software/README.md) for details*

---

## Usage

[Basic usage instructions]

---

## Repository Structure

```
project-name/
├── Build/          # Design files (CAD, PCB, etc.)
├── Docs/           # Documentation
├── Software/       # Code (if applicable)
└── README.md       # This file
```

---

## Contribute

Enjoy building and using your [Project Name]! We look forward to hearing about your experiences and any further improvements you might have.

**Make a pull request or open an issue.**

For interactions in our team and with the community, we apply the [GOSH Code of Conduct](https://openhardware.science/gosh-code-of-conduct/).

---

## License

[LICENSE NAME] by [Author Names] and contributors. This project is Open Source Hardware - please acknowledge us when using the hardware or sharing modifications.

See the [LICENSE](LICENSE) file for details.

---

## Acknowledgments

- [Contributor/Resource 1]
- [Contributor/Resource 2]

---

## Contact

**Wenzel-Lab**: [https://wenzel-lab.github.io/en/](https://wenzel-lab.github.io/en/)  
**LIBRE Hub**: [https://github.com/LIBREhub](https://github.com/LIBREhub)  
**Issues**: [GitHub Issues](https://github.com/wenzel-lab/[project-name]/issues)

---

*Part of the [Wenzel-Lab](https://github.com/wenzel-lab) open-source hardware initiative*

